                ...
                script {
                    withCredentials([usernamePassword(credentialsId: env.VCENTER_CREDENTIALS_ID, usernameVariable: 'VCENTER_USER', passwordVariable: 'VCENTER_PASS')]) {
                        ...
                        docker run --rm \
                            ...
                            pwsh -File /usr/src/app/Change-RAM.ps1 -vCenterServer '${env.VCENTER_SERVER}' -vCenterUser \$VCENTER_USER -vCenterPass \$VCENTER_PASS -Verbose
                        """
                    }
                }
            }
                ...
                script {
                    withCredentials([usernamePassword(credentialsId: env.VCENTER_CREDENTIALS_ID, usernameVariable: 'VCENTER_USER', passwordVariable: 'VCENTER_PASS')]) {
                        ...
                        docker run --rm \
                            ...
                            -e VCENTER_PASS=\$VCENTER_PASS \
                            ${env.DOCKER_IMAGE_NAME}:${env.DOCKER_IMAGE_TAG} \
                            pwsh -File /usr/src/app/Start-VMs.ps1 -vCenterServer '${env.VCENTER_SERVER}' -vCenterUser \$VCENTER_USER -vCenterPass \$VCENTER_PASS -Verbose
                        """
                    }
                }
            }
